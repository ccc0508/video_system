...
class SparseMatrix:
    ...
    def build(self):
        ...
        for row in range(self.num_rows):
            entries = list(self._temp_data.get(row, {}).items())
            # 按列索引排序
    ...
    def get_row(self, row):
        ...
        if not self._built:
            # 未构建时从临时数据返回
            return list(self._temp_data.get(row, {}).items())
    ...
    def get_row_cols(self, row):
        """获取某行的非零列索引集合"""
        if not self._built:
            return set(self._temp_data.get(row, {}).keys())
        start = self.row_ptr[row]
        end = self.row_ptr[row + 1]
